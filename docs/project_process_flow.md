...
|----------|------------------------|-------------------------------|-----------------|
| **1. System Architecture** | Agent decomposition, memory design, clear module boundaries | Uses a **Supervisor Agent** to coordinate specialized agents (RAG, Quiz, Planner, Memory, Safety). Each agent has a single responsibility. Course-scoped design ensures isolation. Memory uses local JSON + optional Supabase with course-based separation. | `src/agents/supervisor.py`, `src/agents/*`, `src/tools/state.py`, `src/retrieval/course_materials.py`, `src/memory/*`, `src/agents/memory_agent.py` |
| **2. Implementation** | Fully working modules, no hardcoded logic | All modules are integrated (auth, RAG, quiz, planner, dashboard, memory). Uses LLM-based logic with deterministic fallback (offline quiz/planner/RAG when needed). | `app.py`, `src/ui/*`, `src/agents/*`, `src/tools/*`, `src/agents/quiz_generator.py`, `src/agents/study_planner.py`, `src/agents/course_rag.py` |
| **3. Agent Collaboration & Intelligence** | Dynamic routing and inter-agent coordination | Supervisor routes queries to correct agents (RAG / Quiz / Planner / Memory / Safety). Agents share state and course context. Quiz results update planner via weak-topic signals. | `src/agents/supervisor.py`, `src/agents/input_router.py`, `src/agents/progress_evaluator.py`, `src/agents/study_planner.py` |
| **4. Memory Quality** | Context-aware adaptive memory system | Multi-layer memory: chat history, summaries, quiz results, weak topics, reminders. Memory directly influences recommendations and planning (adaptive learning loop). | `src/agents/memory_agent.py`, `src/tools/state.py`, `src/tools/quiz_history.py`, `src/agents/progress_evaluator.py` |
| **5. User Interface** | Interactive Streamlit/Gradio UI | Multi-page Streamlit app: login, courses, chat, upload, quiz, planner, dashboard, settings. | `app.py`, `src/ui/chat_page.py`, `src/ui/upload_page.py`, `src/ui/quiz_page.py`, `src/ui/dashboard_page.py` |
---

## Bonus Features

| Feature Requirement | How It is Applied | Files |
|---------------------|------------------|-------|
| Multilingual Support | System supports Arabic + English input/output. Memory and retrieval work across both languages. Includes RTL UI support and translation layer. | `src/localization.py`, `src/ui/theme.py`, `src/tools/planner_localization.py` |
| Prompt Injection Detection | Input validation before LLM routing using rule-based safety checks for adversarial prompts (English + Arabic). | `src/agents/safety_agent.py` |
| Output Filtering / Guardrails | Output inspection layer removes secrets, system leaks, stack traces, and unsafe content before displaying results. | `src/tools/output_filter.py` |
